...
/**
 * A sky reminder has come due (Fase 5): decide whether it is worth posting, post it
 * if so, and arm the next one either way.
 *
 * Also handles `BOOT_COMPLETED`. Alarms do not survive a reboot, so without this the
 * reminders would go quiet after a restart and nothing would say why. The shared
 * periodic job (Fase 6) will re-arm on every fetch too, which makes this the fast
...
class SkyAlarmReceiver : BroadcastReceiver() {
    ...
    /**
     * The whole of [onReceive] minus the dispatch, so a test can await it instead of
     * racing a fire-and-forget coroutine. [context] is already the application one.
     */
    ...
    private suspend fun deliver(context: Context, intent: Intent) {
        ...
        if (SkyReminderPolicy.decide(job, verdict, settings.skyNotifyOnFail) !=
            ...
            // decided on, and re-deciding it at the next re-arm would only produce
