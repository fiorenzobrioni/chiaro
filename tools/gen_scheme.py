#!/usr/bin/env python3
"""Generates ui/theme/Scheme.kt — the two Chiaro fallback schemes (DESIGN.md §2.2, §2.5).
...
Two source sets go through it: the paper palette of §2.2 and the vivid one of §2.5.
One algorithm, two dresses — a second palette picked by hand would be a second
palette to re-measure by hand every time Material moves a tone.
...
# The second dress (DESIGN.md §2.5), added 7 set 2026. Same three times of day, and
# the daylight sky promoted to primary: the reader who picks this one is asking for
# the blue-and-brilliant look, and amber at tone 40 is a bronze whatever else moves.
# Chroma is not pinned on the neutrals here beyond what keeps a surface a surface:
# 6 and 14 give a cool daylight white instead of paper, which is the whole point.
VIVID = {
    "primary": ("#2C7BF2", None),
    "secondary": ("#FFB000", None),
    "tertiary": ("#7B57F0", None),
    "error": ("#E01B24", None),
    "neutral": ("#7C8794", 6.0),
    "neutralVariant": ("#79859B", 14.0),
}

PALETTES = {"Chiaro": PAPER, "Vivid": VIVID}
...
// GENERATED by tools/gen_scheme.py from the source colors of DESIGN.md §2.2 and §2.5.
// Do not hand-edit: change a source there, regenerate, and let PaletteContrastTest
// say whether the result still holds. These schemes are only ever used when dynamic
// color is off or unavailable (ChiaroTheme decides); they are the app looking like
// itself — as warm paper, or as a bright daylight sky — rather than like the wallpaper.
"""


def scheme(sources: dict, index: int, name: str, builder: str) -> str:
...
def main() -> None:
    print(HEADER)
    blocks = []
    for prefix, sources in PALETTES.items():
        blocks.append(scheme(sources, 0, f"{prefix}LightScheme", "lightColorScheme"))
        blocks.append(scheme(sources, 1, f"{prefix}DarkScheme", "darkColorScheme"))
    print("\n\n".join(blocks))
