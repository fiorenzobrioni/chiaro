 ...
 * ever read again**.
 *
 * Everything else on disk is bounded, and each bound is count-based on purpose — a
 * Journal is as deep as its last hundred commits, not as deep as the last fortnight,
 * because a phone that was off for a week must not come back to an empty diary. But a
 * count per place says nothing about places that stopped existing, and this app mints
 * them constantly: a removed city keeps its commits, and the GPS pseudo-city mints a
 * fresh `cacheKey` for every ~1.1 km cell it has ever adopted, each one arriving with
 * its own allowance of a hundred history rows and — until the backstop added beside
 * this — an unbounded number of warning rows. None of it is stale data; all of it is
 * data about a subject the app no longer has.
 *
 ...
 *   or nothing per key: swipe-to-remove has an undo, re-adding a place a reader missed
 *   is one tap, and a Journal that came back with its middle pages gone would be worse
 *   than one that came back empty.
 */
class StoredDataSweep(
    private val historyDao: WeatherHistoryDao,
    private val warningRecordDao: WarningRecordDao,
    ...
    suspend fun run(liveKeys: Set<String>) {
        ...
        warningRecordDao.pruneForeign(keys, cutoff.epochSecond)
    ...
    companion object {
         ...
         * the same week the forecast itself reaches: long enough that an undo, a
         * re-add, or a phone left in a drawer costs nobody their Journal, short enough
         * that a fortnight of commuting does not leave a fortnight of dead cells.
         */
