# The 1.0.0 development record

Chiaro reached 1.0.0 without a release before it, so everything the app does was "added"
once and a great deal of it was then changed, fixed or taken out again along the way.
That full record is here rather than in [CHANGELOG.md](../CHANGELOG.md), which keeps the
readable summary: `release.yml` uses the changelog section as the body of the GitHub
Release, and a thousand lines of development history is not what somebody arriving at a
release page is looking for.

Nothing is summarised away below. This is the record as it was written, entry by entry,
while the work happened. From 1.1.0 on, version sections go back to a normal size and
live in `CHANGELOG.md` alone.
